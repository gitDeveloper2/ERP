...
public Population evolve(Population population){return mutatePopulation(crossoverPopulation(population));}
	Population crossoverPopulation(Population population){
		...
		IntStream.range(0,(int) Driver.NUMB_OF_ELITE_SCHEDULES).forEach(x->crossoverPopulation.getSchedules().set(x, population.getSchedules().get(x)));
	IntStream.range((int) Driver.NUMB_OF_ELITE_SCHEDULES,population.getSchedules().size()).forEach(x->{
...
Population mutatePopulation (Population population){
	...
	IntStream.range(0,(int) Driver.NUMB_OF_ELITE_SCHEDULES).forEach(x-> schedules.set(x,population.getSchedules().get(x)));
	IntStream.range((int) Driver.NUMB_OF_ELITE_SCHEDULES,population.getSchedules().size()).forEach(x->{ schedules.set(x,mutateSchedule(population.getSchedules().get(x)));
	});
...
Population selectTournamentPopulation (Population population){
	Population tournamentPopulation = new Population ((int) Driver.TOURNAMENT_SELECTION_SIZE,data);
	IntStream.range(0,(int) Driver.TOURNAMENT_SELECTION_SIZE).forEach(x->{
